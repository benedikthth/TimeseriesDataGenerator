...
# def sigmoid(x):
#     # return -1 / ( 1 + math.pow(math.e, (-x+0.5)*8 )  ) + 1
#     return x * x

# VARIABLES
# sampling_rate = 100
# recording_length = 30 # in seconds
# seq_length = sampling_rate * recording_length # how many recordings will the dataset contain


#how many different frequecies overlaid on the signal. 
# num_freqs = 2
# amplitude = 1
# the first wave will have the amplitude of 1, the next will have amplitude of 1 * amplitude dropoff and so and so forth


# def gen_dataset(how_many=100, variant_over_time=True, return_not=False, num_freqs = 1, num_outputs=1, amplitude_dropoff=0.5, st_deviation=1, freq_range=(2.5, 50), sampling_frequency=100, sequence_length=30, generator_settings=None):  
    
#     """Create dataset containing how_many synthesized signals of length {recording_length} 
#     with sampling rate {sampling_rate}. The signal has zero mean normally distrubuted noise
#     with standard deviation {st_deviation}. The 'strength' of the noise also changes over time
#     using a perlin noise function. 

#     Parameters:
#         how_many (int): how many observations(signals) to generate\n
#         variant_over_time (bool): Should the noise strength change over time?\n
#         return_not (bool): If true then this function returns triple (labels, data, noise_strengths)\n
#         num_freqs: how many frequencies are overlaid on eachother.\n
#         num_outputs: how many predictions to make. First prediction will be sine of stronges amplitude.\n
#         amplitude_dropoff: how much less amplitude the subsequent sine waves will have. (sine_i * amplitude_dropoff^i)\n
#         generator_settings: Object containing all these settings. Must contain all settings, will overwrite all other settings.\n
...
#     Returns:
#         triple contining labels(list of numbers), data (list of lists of data points), (if return_not) noise_over_time (list of list of noise strengths.)
#     """
#     #A sine wave has a cycle freq. of 2PI
#     hz = 2 * math.pi # one cycle

#     perlin_scale_range = (0.01, 0.05)
#     _psrange = perlin_scale_range[1] - perlin_scale_range[0]
...
#         how_many = generator_settings['how_many']
#         num_freqs = generator_settings['num_freqs']
#         num_outputs = generator_settings['num_outputs']
#         amplitude_dropoff = generator_settings['amplitude_dropoff']
#         st_deviation = generator_settings['st_deviation']
#         freq_range = generator_settings['freq_range']
#         sampling_frequency = generator_settings['sampling_frequency']
#         sequence_length = generator_settings['sequence_length']

#     if num_freqs < num_outputs: 
#         raise Exception(f'Num freqs should not be less than num_outputs. ({num_freqs} < {num_outputs})')
#         return None, None

#     _frange = freq_range[1] - freq_range[0]
    
#     samples = sequence_length * sampling_frequency 
...
#         if(return_not):
#             noisemaps.append(noise_ot)

#         y = np.arange(sequence_length, step=1/sampling_frequency)
#         #generate a frequency in the range of freq_range[0] - freq_range[1]
#         freqs  = [(_frange*random.random() ) + freq_range[0] for i in range(num_freqs)]
#         if num_freqs != 1:
#             labels.append(freqs[:num_freqs])
#         else:
#             labels.append(freqs[0])
#         # the frequency of a normal sine is 1/2pi, so to get the 
#         # sine wave to a frequency that we want we must multiply 2pi by our desired frequency
...
#         freqs = [ f * hz for f in freqs ]
#         # Generate a random offset.
#         offsets = [( _orange*random.random() ) + offset_range[0] for i in range(num_freqs)]
#         # Function that generates the sine wave.
#         f = (
#             lambda t, f, o: math.sin( ( (t) *  f ) + o  )
#         )
#         #precompute amplitude dropoffs.
#         ads = [pow(amplitude_dropoff, i) for i in range(num_freqs)]
#         # create base sine. 
#         x = [f(p, freqs[0], offsets[0]) for p in y]
#         # add sines.
#         if num_freqs != 1:
...
#         #create random numbers normally distributed with 0 mean.
#         rands = np.random.normal(0, st_deviation, len(x))
#         #multiply random noise by the perlin noise to gradually decrease, increase noise
#         rands = list(map(mul, noise_ot, rands))
#         #add the new nosie to the measurements.
#         x = np.array(list(map(add, x, rands)) )
...
#     if return_not:
#         return np.array(labels), np.array(dataset), np.array(noisemaps)
